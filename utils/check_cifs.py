...
def check_cif_files(directory: str, n_cpu: Optional[int] = 1) -> None:
    ...
    with multiprocessing.Pool(processes=n_cpu) as pool:
        check_and_remove_partial = partial(check_and_remove_corrupt_cif, directory=directory)
        bad_files_count = sum(tqdm(pool.imap_unordered(check_and_remove_partial, cif_files), total=len(cif_files)))
...
def check_and_remove_corrupt_cif(file: str, directory: str) -> int:
    ...
    try:
        ...
        pdf_calculator = PDFCalculator(rmin=0, rmax=30, rstep=0.1, qmin=0.7, qmax=20, qdamp=0.04, delta2=2)
...
if __name__ == '__main__':
    ...
    parser.add_argument("directory", type=str, help="Absolute path to the folder containing the CIFs.")
