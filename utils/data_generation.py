...
class sim_pdfs:
    ...
    def __init__(
        ...
        cif_dir: str,
        save_dir: str,
        sim_range: Dict[str, List[Union[float, None]]],
        split: int,
    ):
        ...
        self.gen_pdfs(
            file,
            [
                sim_range[key][0] if sim_range[key][0] is not None else -1
    ...
    def init_df(self, file: str) -> None:
        ...
        r = np.arange(
            self.par_dict["rmin"], self.par_dict["rmax"], self.par_dict["rstep"]
        )
    ...
    def gen_pdfs(self, cluster_file: str, parameter: dict, index: int) -> None:
        ...
        pdf_calc = PDFCalculator(
            rmin=self.par_dict["rmin"],
            rmax=self.par_dict["rmax"],
            rstep=self.par_dict["rstep"],
            qmin=parameter[0],
            qmax=parameter[1],
            qdamp=parameter[2],
            delta2=parameter[3],
        ...
        ph_row = np.concatenate(
            (
                [
                    cluster_file.rsplit(".", 1)[0] + "_{:05d}".format(index),
                    a,
                    b,
                    c,
                    alpha,
    ...
    def lattice(self, stru, parameter):
        a, b, c = (
            self.apply_relative_percentage_change(stru.lattice.a, parameter[6]),
...
def get_structures(directory: str, savedir: str, split: int) -> List[str]:
    ...
    return [
        ...
        if file.rsplit(".")[0] + ".csv" not in existing_files
    ]


def simulate_pdfs(
    simulation_range, stru_path: str, n_cpu: int = 1, n_simulations: int = 10
) -> str:
    """
    ...
    with multiprocessing.Pool(processes=n_cpu) as pool:
        sim_pdfs_partial = partial(
            sim_pdfs,
            cif_dir=stru_path,
            save_dir=savedir,
            sim_range=simulation_range,
            split=n_simulations,
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Simulate PDFs using given structures and parameters."
    )
    parser.add_argument(
        "stru_path",
        type=str,
        help="The directory where the structure files are located.",
    )
    parser.add_argument(
        "-n",
        "--n_cpu",
        type=int,
        default=1,
        help="(Optional) The number of CPUs to use for simulation. Default is 1.",
    )
    parser.add_argument(
        "-s",
        "--n_simulations",
        type=int,
        default=10,
        help="(Optional) The number of simulations to run for each structure. Default is 10.",
    )
